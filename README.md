...
- **Squad Builder** — four modes, all rendered on a real FPL-style pitch layout (starting XI positioned by row — GK, then DEF/MID/FWD moving up the pitch, formation read directly from the squad — with the 4 bench players shown separately below; the full table is also available in a collapsed expander). The squad's single highest-predicted_points player gets a ⭐ badge — labeled **MVP** (highest points-per-90 that season) for a Team of the Season squad, or **Player of the Week** (top predicted scorer that gameweek) for a Historical gameweek squad. No badge is shown for pre-season/manual-entry squads, since predicted_points there is last season's closing form, not a real per-gameweek or full-season figure — an MVP/POTW label would overstate what that number means.
  ...
  - *Team of the Season* — a LOOK BACK at an already-completed season, not a prediction (the season is over, so there's nothing left to predict): the optimal squad using each player's real **points-per-90-minutes** across every gameweek they played that season, at their final-gameweek price. Per-90, not per-38-gameweeks, so a player who missed matches to injury/rotation isn't unfairly penalized against one who started every game. Players under 450 total minutes that season are excluded — too small a sample for a trustworthy rate (a single 1-minute cameo goal would otherwise look like a 90-point-per-90 season).
...
- Cross-check the dashboard's Team of the Season / Team of the Week squads against FPL's own published "Dream Team" (a real per-gameweek feature on the live FPL site/API — the top scorer in each position, flagged via `in_dreamteam` on `element-summary`). Checked: this project's 10-season historical dataset (vaastav's merged CSVs, what `features.parquet` is built from) does **not** carry that flag, so there's currently no column to compare against without a separate collector pass against the live API's per-gameweek history for every player, which isn't wired up yet. Worth adding as an explicit sanity check once built, since it's a free, ready-made reference point already computed by FPL itself.
