...
- **Manager History** — headed by the manager's real name (read from the collector's saved `entry/{id}/info.json` — `snapshot_entry()` now fetches this alongside history/picks — falling back to the bare numeric entry id if unavailable, rather than a display nicety breaking the rest of the tab), then three sections:
  ...
  All three read live from the collector's own saved files, not a hardcoded table (the tab used to carry a copy-pasted static snapshot that would silently go stale — fixed). `data/raw/` is gitignored, so a fresh deploy falls back to `data/dashboard_entry_history.json` and `data/dashboard_entry_info.json` for the past-seasons/name sections — single, deliberately-committed, non-timestamped copies of this same manager's data, same fallback pattern as `data/dashboard_bootstrap.json`, not a new privacy decision since this exact data is already public on the linked manager-history page. "My current squad" and League Tracker deliberately have **no** committed fallback — a live gameweek's picks/points/standings change constantly while it's in progress, so a stale committed copy would misrepresent old numbers as current rather than being a reasonable approximation; they simply show an explicit "not collected yet" message in an environment without `data/raw/`.
- **League Tracker** — real standings for every PRIVATE classic (points-based) mini-league this manager has joined by code, read from `entry/{id}/leagues/{league_id}.json` (FPL's own `leagues-classic/{id}/standings` endpoint via the collector). Deliberately excludes FPL's own auto-generated global/region/favourite-club leagues (`league_type == "s"`) — those aren't leagues a manager actually "joined." This manager's own row is highlighted in the standings table. A league selector lets you switch between every private league you're in.
