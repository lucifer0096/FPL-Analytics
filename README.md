...
- **Chip Advisor** — placeholder for now: chip timing genuinely needs a run of several **upcoming** gameweeks to project against, which doesn't exist yet this early in a live season. Points to the Historical & Model page's Squad Builder → Historical gameweek mode for a demonstration of how this tool works once enough real fixtures exist.
...
- **Squad Builder** — five modes, all rendered on the same real FPL-style pitch layout (starting XI positioned by row — GK, then DEF/MID/FWD moving up the pitch, formation read directly from the squad — with the 4 bench players shown separately below; the full table is also available in a collapsed expander). Each player card shows their real FPL headshot, fetched directly from FPL's own photo CDN (`resources.premierleague.com/premierleague/photos/players/110x140/p{code}.png`, verified to return a real image for a real player code) keyed off `code` — FPL's stable cross-season player id, not the season-scoped `element`/`id` (see the `element` caveat in Historical Training Data) — so a photo resolves correctly even for a live-pool player whose current-season numeric id differs from their real, stable code. Falls back to the player's real team badge (also fetched from FPL's own CDN) if a specific player genuinely has no photo there. The squad's single highest-predicted_points player gets a ⭐ badge — labeled **MVP** (most points scored that season/window) for a Team of the Season squad, or **Player of the Week** (top predicted scorer that gameweek) for a Historical gameweek squad. A separate ✓ badge marks FPL's own official Team of the Week picks (`in_dreamteam` — see Historical Training Data) wherever that data exists. No ⭐ badge is shown for pre-season/manual-entry squads, since predicted_points there is last season's closing form, not a real per-gameweek or full-season figure — an MVP/POTW label would overstate what that number means.
  ...
  - *Team of the Season* — a LOOK BACK at an already-completed season (or a narrower gameweek window — "Team of the Week" — via a slider), not a prediction: the optimal squad ranked by each player's real **total points scored** across the window, at their final-gameweek price. Ranked by total, not a rate — a great points-per-game over a handful of appearances can't outrank a player who played most of the window and produced far more for a real squad. `points_per_game` (matching FPL's own published metric exactly) is shown on each card as context, not as what drives selection.
  - *2026-27 pre-season* — build a squad recommendation using each player's **live current price** paired with their **rolling-5 average at the end of 2025-26** as a predicted-points stand-in. Deliberately NOT run through the xP model — there's no real 2026-27 fixture list to build `fixture_difficulty` from until enough of the season has been collected.
  - *Scout Picks (2026-27 season opener)* — this project's own take on FPL's editorial "Scout Picks" article, NOT a scrape of it (checked directly: not structured API data, no stable weekly URL). Built from real, verified boosts: an easy GW1 fixture against a confirmed promoted team, and confirmed set-piece duty via FPL's own `penalties_order` field. Every boosted player gets a shown, checkable reason.
  - *My squad (enter manually)* — pick your own real 15-man squad by name and see it validated and rendered the same way as an optimizer-built squad.
