...
- **Squad Builder** — three modes, all rendered on a real FPL-style pitch layout (starting XI positioned by row — GK, then DEF/MID/FWD moving up the pitch, formation read directly from the squad — with the 4 bench players shown separately below; the full table is also available in a collapsed expander):
...
- **Transfers** — using the squad just built, find the best transfer(s) into the following gameweek. Only available for squads built in *Historical gameweek* mode — pre-season and manually entered squads have no "next gameweek" of historical data to check against yet.
